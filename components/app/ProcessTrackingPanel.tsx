...
export function ProcessTrackingPanel({
  ...
  const top4 = routeOptions.length
    ? routeOptions
        .filter((route) =>
          inferredType === 'swap' && enforceSwapWhitelist
  ...
  const protocolsToCheck = useMemo(() => protocolNames.slice(0, 5), [protocolNames]);

  const protocolFeeMap = useMemo(
    () =>
      new Map(routeOptions.map((route) => [route.mainTool, route.gasCostUSD ? `$${route.gasCostUSD}` : 'N/A'])),
    [routeOptions]
  );
  ...
  const handleChoose = async (index: number) => {
    ...
    const selectedRoute = routeOptions[index] ?? null;
    if (!selectedRoute) {

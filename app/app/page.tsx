...
export default function AppPage() {
  ...
  const send = async () => {
    ...
    const processingMsg: Message = {
      id: processingId,
      role: 'assistant',
      content: 'Your request is being processed. Check the tracking panel on the right for progress.',
      at: new Date().toISOString(),
    };
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === processingId ? { ...m, content: 'Request failed. You can close the tracking panel and try again.' } : m
        )
      );
    } finally {
      setLoading(false);
  ...
  const handleTrackingComplete = (result: FinalResult | null) => {
    ...
    const content = `Done. ${result.protocol} — Pair: ${result.pair}, Fee: ${result.fee}. Input: ${result.inputAmount} → Output: ${result.outputAmount}. Tx: ${result.txHash.slice(0, 10)}… Approval: ${result.approvalHash.slice(0, 10)}…`;
    setMessages((prev) =>
      prev.map((m) =>
        m.id === processingMessageId ? { ...m, content, result } : m
      )
    );
  ...
  return (
    <div className="h-screen overflow-hidden bg-white flex flex-col">
      ...
      <div className="relative flex-1 flex flex-col min-h-0 pt-16">
        <div className={`relative z-10 flex-1 flex min-h-0 gap-3 px-4 py-2 ${trackingOpen ? 'flex-row' : 'flex-col'}`}>
          {/* Chat: left when tracking open, centered full width when closed */}
          <div
          ...
          >
            {/* Messages - circles design in this layer, behind "Send a message to get started." */}
            <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0 overscroll-contain relative">
              ...
              {messages.map((m) => (
                ...
                >
                  <div className={`flex gap-2 ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                    ...
                    {m.result && (
                      <div className="mt-3 pt-3 border-t border-gray-200/80 space-y-1.5 text-xs">
                        <p><strong>Protocol:</strong> {m.result.protocol}</p>
                        <p><strong>Pair:</strong> {m.result.pair}</p>
                        <p><strong>Fee:</strong> {m.result.fee}</p>
                        <p><strong>Input:</strong> {m.result.inputAmount} → <strong>Output:</strong> {m.result.outputAmount}</p>
                        <p className="break-all"><strong>Tx:</strong> {m.result.txHash}</p>
                        <p className="break-all"><strong>Approval:</strong> {m.result.approvalHash}</p>
                      </div>
                    )}
                    {m.report && !m.result && (
                      <div className="mt-3 pt-3 border-t border-gray-200/80 space-y-2 text-xs">
                        <p><strong>Accuracy:</strong> {m.report.accuracy}</p>
                        <p><strong>Gas (est.):</strong> {m.report.gasFeeEstimate}</p>
                        <p><strong>Optimistic:</strong> {m.report.optimisticEstimate}</p>
                        <div className="grid grid-cols-2 gap-2 mt-2">
                          <div>
                            <p className="font-medium text-foreground">Top bridges</p>
                            <ul className="list-disc list-inside text-muted-foreground">
                              {m.report.topBridges.slice(0, 5).map((b, i) => (
                                <li key={i}>{b.name} ({b.score})</li>
                              ))}
                            </ul>
                          </div>
                          <div>
                  ...
                  <div className={`mt-1 flex items-center gap-2 px-1 text-[10px] text-muted-foreground ${m.role === 'assistant' ? 'pl-9' : 'justify-end'}`}>
          ...
          {/* Process tracking panel - right side when open */}
          {trackingOpen && (

...
function getRouteType(route) {
  ...
  return steps.some((s) => s?.type === "cross") ? "bridge" : "swap";
}
function getMainTool(route, type) {
  const steps = Array.isArray(route?.steps) ? route.steps : [];
  const preferred = type === "bridge" ? "cross" : "swap";
  const match = steps.find((s) => s?.type === preferred) ?? steps[0];
...
function processBridge(nodeRuntime) {
  ...
  const routeSummaries = routes.map((route) => {
    ...
    if (!executionDuration || executionDuration <= 0) {
      ...
      let mod = 1;
      if (tool.includes("stargate"))
        mod = 0.5;
      if (tool.includes("hop"))
        mod = 0.8;
  ...
  const now = Date.now();
  const workflow2 = [
    { name: "Intent parsed", status: "ok", details: "Bridge request detected", timestamp: now },
    { name: "Quote requested", status: "ok", details: `Requested ${routes.length} route(s) from LI.FI`, timestamp: now + 50 },
    { name: "Protocol validation", status: "ok", details: "Bridge protocols checked", timestamp: now + 100 },
...
function onTrigger(runtime2) {
  ...
  runtime2.log("[3] Protocol validation — bridge.");

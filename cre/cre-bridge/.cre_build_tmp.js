...
function parseIntent(prompt, fromAddress) {
  ...
  if (fromChainId === 56)
    fromTokenName = "bnb";
  let toTokenName = "eth";
  if (toChainId === POLY_CHAIN_ID)
    toTokenName = "matic";
  if (toChainId === AVAX_CHAIN_ID)
    toTokenName = "avax";
  if (toChainId === 56)
    toTokenName = "bnb";
  let foundFrom = false;
...
function processBridge(nodeRuntime) {
  ...
  const routeSummaries = routes.map((route) => {
    ...
    const reliabilityScore = highRel.has((getMainTool(r) || "").toLowerCase()) ? "99.9%" : "98.5%";
    return {
      id: r.id,
      type: "bridge",

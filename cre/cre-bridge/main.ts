...
function parseIntent(prompt: string, fromAddress: string): Record<string, unknown> {
  ...
  for (const [name, id] of Object.entries(CHAINS)) {
    ...
    if (idx !== -1) foundChains.push({ id, index: idx });
  }
...
function processBridge(nodeRuntime: NodeRuntime<Config>): BridgeProcessResult {
  nodeRuntime.log("cre-bridge: processBridge START.");
  try {
    const httpClient = new HTTPClient();
    const base = nodeRuntime.config.apiBaseUrl;
    nodeRuntime.log(`cre-bridge: apiBaseUrl is ${base}`);
    ...
    const now = Date.now();
    const workflow = [
      { name: "Intent parsed", status: "ok" as const, details: `Bridge request detected: ${fromSymbol} ${payload.fromChainId === BASE_CHAIN_ID ? "Base" : "Arbitrum"} → ${payload.toChainId === ARB_CHAIN_ID ? "Arbitrum" : "Base"}`, timestamp: now },
      { name: "Quote requested", status: "ok" as const, details: `Fetched ${routes.length} live bridge route(s) from LI.FI`, timestamp: now + 50 },
      { name: "Protocol validation", status: "ok" as const, details: `Bridge whitelist applied: ${ALLOWED_BRIDGE_PROTOCOLS.join(", ")}`, timestamp: now + 100 },
    ...
    httpClient.sendRequest(nodeRuntime, { url: `${base}/api/maima`, method: "POST", headers: { "Content-Type": "application/json" }, body: creReportBody }).result();
    nodeRuntime.log("cre-bridge: Report submitted.");
    return { processed: true, requestId: request.id, timestamp: Date.now() };

  } catch (e: any) {
    nodeRuntime.log(`cre-bridge: FATAL ERROR in processBridge: ${e?.message || e || 'Unknown error'}`);
    return { processed: false, requestId: "", timestamp: Date.now() };
  }
}
...
function onTrigger(runtime: Runtime<Config>): BridgeProcessResult {
  runtime.log("cre-bridge workflow: onTrigger START.");
  try {
    const result = runtime.runInNodeMode(processBridge, consensusIdenticalAggregation<BridgeProcessResult>())().result();
    runtime.log("cre-bridge workflow: processBridge completed.");
    return result;
  } catch (err: any) {
    runtime.log(`cre-bridge workflow: ERROR in onTrigger: ${err?.message || err || 'Unknown'}`);
    throw err;
  }
}

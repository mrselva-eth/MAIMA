...
function fetchRequests(nodeRuntime) {
...
function onCronTrigger(runtime2) {
  runtime2.log("MAIMA workflow triggered.");
  const out = runtime2.runInNodeMode(fetchRequests, consensusIdenticalAggregation())().result();
  const n = out.activeRequests ?? 0;
  runtime2.log("[1] Intent parsed — " + (n > 0 ? "request(s) detected" : "no active requests") + ".");
  runtime2.log("[2] Quote requested — GET /api/maima/queue returned " + n + " request(s).");
  runtime2.log("[3] Protocol validation — whitelist check (staging).");
  runtime2.log("[4] Ranking — sorted by lowest gas fee (USD).");
  runtime2.log("[5] Selection — reportGenerated: " + (out.reportGenerated ?? n > 0) + ".");
  return {
    ...out,
    timestamp: Date.now()
  };

...
function toBase64(str) {
  ...
  for (let i2 = 0;i2 < bytes.length; i2 += 3) {
    ...
    result += i2 + 1 < bytes.length ? key[(b & 15) << 2 | c >> 6] : "=";
    result += i2 + 2 < bytes.length ? key[c & 63] : "=";
  }
  return result;
}
function orchestrate(nodeRuntime) {
  ...
  for (const req of requests) {
    ...
    if (reportData.success && reportData.report != null)
      continue;
...
function fetchCongestionMetrics(nodeRuntime) {
  const httpClient = new ClientCapability;
  ...
  for (let i2 = 0;i2 < urls.length; i2++) {
    const rpcUrl = urls[(startIndex + i2) % urls.length];
    try {
  ...
  if (!metrics)
    throw new Error("All RPCs failed for congestion metrics");
  return metrics;
}
...
function onCronTrigger(runtime2) {
  runtime2.log("MAIMA main workflow tick.");
  let congestionData = null;
  try {
    const metrics = runtime2.runInNodeMode(fetchCongestionMetrics, ConsensusAggregationByFields({
      blockNumber: median,
      utilization: median,
      baseFeeGwei: median,
      timestamp: ignore
    ...
    congestionData = {
      chainId: runtime2.config.chainId,
    ...
    httpClient.sendRequest(runtime2, {
      url: `${runtime2.config.apiBaseUrl}/api/maima`,
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: toBase64(JSON.stringify({ action: "congestion-report", data: congestionData }))
    }).result();
    runtime2.log(`[Congestion] Status: ${status} (${congestionData.utilization}%)`);
  } catch (err) {
    runtime2.log(`[Congestion] Error: ${err}`);
  }
  let orchResult = { delegatedCount: 0, timestamp: Date.now() };
  try {
    orchResult = runtime2.runInNodeMode(orchestrate, consensusIdenticalAggregation())().result();
    runtime2.log(`[Orchestrator] Queue polled. Delegated ${orchResult.delegatedCount} requests.`);
  } catch (err) {
    runtime2.log(`[Orchestrator] Error: ${err}`);
  }

...
function getMainTool(route) {
  ...
  const match = steps.find((s) => s?.type === "swap") ?? steps[0];
...
function processSwap(nodeRuntime) {
  ...
  if (toSymbol && toSymbol !== fromSymbol) {
    try {
      ...
      const prData = JSON.parse(new TextDecoder().decode(pr.body));
      if (prData.price != null)
        verifiedPrices.push({ symbol: toSymbol, price: `$${Number(prData.price).toFixed(2)}`, updatedAt: prData.updatedAt0 });
    } catch {}
  }
  const routeSummaries = routes.map((route) => {
    ...
    if (!executionDuration || executionDuration <= 0) {
      ...
      let mod = 1;
      if (tool.includes("uniswap") || tool.includes("1inch"))
        mod = 0.7;

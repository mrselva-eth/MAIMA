...
function onTrigger(runtime2) {
  ...
  runtime2.log("[1] Intent parsed — swap request detected.");
  runtime2.log("[2] Quote requested — polling /api/maima/queue.");
  const result = runtime2.runInNodeMode(runSwap, consensusIdenticalAggregation())().result();
  runtime2.log("[3] Protocol validation — swap protocols checked.");
  runtime2.log("[4] Ranking — sorted by gas fee.");
  runtime2.log("[5] Selection — " + result.route + ", slippage " + result.slippage + ", priceImpact " + result.priceImpact + ".");
  return result;
}
async function main() {
